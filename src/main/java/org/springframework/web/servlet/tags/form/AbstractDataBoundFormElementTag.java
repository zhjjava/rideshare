...
public abstract class AbstractDataBoundFormElementTag extends AbstractFormTag implements EditorAwareTag {
	...
	protected void writeDefaultAttributes(TagWriter tagWriter) throws JspException {
		...
		//add by mz, 20150715 begin
		writeFieldErrorContent(tagWriter);
 ...
 */
	 ...
	 * @throws JspException
	 */
	protected void writeFieldErrorContent(TagWriter tagWriter) throws JspException {
        if(!shouldRender()){
        	return;
        }
		String delimiter = ObjectUtils.getDisplayString(evaluate("delimiter", ","));
		String[] errorMessages = getBindStatus().getErrorMessages();
		String errorMessage="";
		for (int i = 0; i < errorMessages.length; i++) {
			if (i > 0) {
				errorMessage = errorMessage + (delimiter);
			}
			errorMessage = errorMessage + getDisplayString(errorMessages[i]);
		}
		tagWriter.writeAttribute("title", errorMessage);
		tagWriter.writeAttribute("style", "background-color:yellow");
	}	
}
