...
public class FormTag extends AbstractHtmlElementTag {
	...
	protected int writeTagContent(TagWriter tagWriter) throws JspException {
       ...
       //add by mz 20150716 begin
		writeActionMsgList();
		writeActionErrors();
	       //add by mz 20150716 end		
		return EVAL_BODY_INCLUDE;
	}
	/**
	 * Writes action message set in controllers.
	 * add by mz 20150716
	 */
	private void writeActionMsgList() throws JspException {
		List actionMsgList = (List)this.pageContext.getAttribute(ACTION_MSG_LIST,PageContext.REQUEST_SCOPE);
		if (actionMsgList != null && actionMsgList.size()>=0) {
			this.tagWriter.appendValue("<div>\n");
			for (Object msg : actionMsgList) {
				this.tagWriter.appendValue("<li><span class=\"actionMsgSpan\">"+msg+"</span></li>\n");
			}
			this.tagWriter.appendValue("</div>");
		}
	}
	 ...
	 * add by mz 20150716
	 */
	private void writeActionErrors() throws JspException {
		Errors errors =  super.getBindStatus().getErrors();
		if(errors==null){
			return;
		}
		List actionErrorList = errors.getGlobalErrors();
		if (actionErrorList != null && actionErrorList.size()>0) {
			this.tagWriter.appendValue("<div>\n");
			for (Object msg : actionErrorList) {
				this.tagWriter.appendValue("<li><span class=\"actionErrorsSpan\">"+msg+"</span></li>\n");
			}
			this.tagWriter.appendValue("</div>");
		}
	}
