...
public abstract class TableWriterTemplate
{
    ...
    public void writeTable(TableModel model, String id) throws JspException
    {
        try
        {
        	//log.debug("ssssssssssssssss:" + model.isRenderPagingBannerPlacement());
            ...
            if (noItems && !properties.getEmptyListShowTable())
            {
                ...
                if(properties.getAutoGeneratePagingFields() && model.isRenderPagingBannerPlacement()){
                    writeSomeHiddenFields(model);
                 }
                return;
            }

            // Put the page stuff there if it needs to be there...
            if (properties.getAddPagingBannerTop() && model.isRenderPagingBannerPlacement())
            {
            ...
            // Put the page stuff there if it needs to be there...
            if (properties.getAddPagingBannerBottom() && model.isRenderPagingBannerPlacement())
            {
                // search result and navigation bar
            	writeBottomBanner(model);
            }            
            //if needs to generate paging and sorting feilds in the form on front JSP page
            //add by MZ 2015-6-24
            //log.debug("properties.getAutoGeneratePagingFields():" +properties.getAutoGeneratePagingFields());
            if(properties.getAutoGeneratePagingFields()&& model.isRenderPagingBannerPlacement()){
               writeSomeHiddenFields(model);
            }
            if (log.isDebugEnabled())
     ...
     * @throws Exception
     */
    protected abstract void writeSomeHiddenFields(TableModel model) throws Exception;
