...
public class HtmlTableWriter extends TableWriterAdapter
{
    ...
    //for generate hidden fields on the JSP page.
    //add by MZ
    ...
    {
        //we don't need displaytag to render the js; mz
    ...
    /**
     * generally avoid developer to repeat writing some same code
     * 
     * write some hidden fields in the form on the front JSP page, such as pageNumber, 
     * sortDirection,sortCriterion etc
     * add by mz
     * @see org.displaytag.render.TableWriterTemplate#writeSomeHiddenFields(org.displaytag.model.TableModel)
     * @param model
     */
    protected void writeSomeHiddenFields(TableModel model)
    {
    	writeHiddenFieldsForPaging();
    	writeHiddenFieldsForSorting();
    }
    //add by mz
    private void writeHiddenFieldsForPaging()
    {
        ...
        pageNumber =this.paginatedList.getPageNumber();
        
    	String formName =this.tableModel.getForm();    
    	if (formName == null)
        {
    		formName =String.valueOf(System.currentTimeMillis());
         }
        // Put the page stuff there if it needs to be there...
        String pageNumberField ="<input type=\"hidden\" name=\"pageNumber\" value=\""
        	+ pageNumber+"\" id=\""+formName+"_pageNumber\"/>" ;  
        //log.debug("pageNumberField:" + pageNumberField);
        write(pageNumberField);
    } 
    //add by mz
    private void writeHiddenFieldsForSorting()
    {
        if (this.paginatedList == null)
        {
        	return;
        ...
        // Put the page stuff there if it needs to be there...
        String sortCriterionField ="<input type=\"hidden\" name=\"sortCriterion\" value=\""
        	+ this.sortCriterion+"\" id=\""+formName+"_sortCriterion\"/>" ;  
        String sortDirectionField ="<input type=\"hidden\" name=\"sortDirection\" value=\""
        	+ this.sortDirection+"\" id=\""+formName+"_sortDirection\"/>" ;  
        
        //added by mz 20150702, in lancang project, system expected a closed div tag
        sortDirectionField= sortDirectionField +"</div>";
        
        write(sortCriterionField);
        write(sortDirectionField);
    } 
    /**
    ...
    private Href getSortingHref(HeaderCell headerCell)
    {
        ...
        else
        {
            ...
            // for generate hidden field on JSP PAGE ADD by mz
            ...
            if (headerCell.isAlreadySorted())
            {
                ...
                // for generate hidden field on JSP PAGE ADD by mz
                this.sortDirection = tableModel.isSortOrderAscending() ? properties.getPaginationAscValue():properties.getPaginationDescValue();
            }
            else
            {
                dirParam = properties.getPaginationAscValue();
                // for generate hidden field on JSP PAGE ADD by mz
                this.sortDirection = properties.getPaginationAscValue();
            }
